        ...
        while not self.move_on:
            ...
            if self.display_mission_cycle:
                ...
                for index, row in settings_df.iterrows():
                    ...
                    times, forces = self.create_waveform(row['Force Min'], row['Force Max'], row['Duration'], times, row['Force Phase'])
                    ...
                    for count, force in enumerate(forces):
                        if count == 0:
                            continue
                        if round(force, 0) != round(forces[count - 1], 0):
                    ...
                    for count in reversed(range(len(times))):
                        if not angle_changes[count] and not force_changes[count]:
                    ...
                    if not force_changes[-1]:
                        force_times.append(saved_last_time)
                        force_values.append(forces[-1])
                        delay.append(0)
                        times = np.append(times, saved_last_time) 
        ...
        for activity in settings_df.index:
            ...
            for count, time in enumerate(raw_activities[activity][1]):
                ...
                if time in raw_activities[activity][3]:
                    ...
                    function_start += 1
                    delay = (raw_activities[activity][6][count])*1000 - 10
                else:
                    delay = (raw_activities[activity][6][count])*1000

                force = raw_activities[activity][2][count]
                file_content.insert(function_start, f'  set_load({round(force, 0)}, {int(delay)});\n')
                function_start += 1
