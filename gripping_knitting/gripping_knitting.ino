...
String activities = "Gripping, Knitting, Typing";
...
else if (instruction.equals("Gripping")){
  activity = 2;
}
else if (instruction.equals("Knitting")){
  activity = 3;
}
else if (instruction.equals("Typing")){
  activity = 4;
}
...
// FUNCTIONS
// GET MESSAGE FROM COMPUTER WITH TIMEOUT
String read_serial(unsigned long timeout) {
  String incoming = "";
...
/// TESTING FOR PAUSE
void test_pause() {
  String instruction = read_serial(50);
  Serial.println(instruction);
  if (instruction.equals("PAUSE")){
    ...
    while (wait_til_play){
      String play_command = read_serial(50);
...
void Gripping(){
  ...
  turn_steps(-13.0);
  delay(100);
  Serial.println("Activity Complete");
}
void Knitting(){
  turn_steps(0.0);
  delay(100);
